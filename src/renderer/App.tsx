...
function App() {
  ...
  const handleLaunch = useCallback(
    async (browserId: string) => {
      ...
      try {
        const result = await globalThis.electronAPI.launchBrowser(browserId, url)
        if (!result.success) {
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'ArrowLeft':
          e.preventDefault()
          setSelectedIndex((prev) => (prev > 0 ? prev - 1 : browsers.length - 1))
          break
        case 'ArrowDown':
        case 'ArrowRight':
          e.preventDefault()
          setSelectedIndex((prev) => (prev < browsers.length - 1 ? prev + 1 : 0))
          break
        case 'Enter':
          e.preventDefault()
  ...
  const getBrowserIcon = (browserId: string) => {
    ...
    return iconMap[browserId] ?? '🌍'
  }

  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="animate-pulse text-white text-lg">Loading browsers...</div>
      </div>
    )
  }
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="bg-widget-bg backdrop-blur-widget border border-widget-border rounded-2xl shadow-widget p-8 max-w-md">
          <h2 className="text-xl font-semibold text-white mb-4">No Browsers Found</h2>
  ...
  return (
    <div className="h-screen w-screen">
      <div className="bg-gray-900/80 backdrop-blur-2xl shadow-2xl p-6 h-full w-full flex flex-col justify-center">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-sm font-medium text-gray-400 mb-2">Open link in:</h1>
          {url && (
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {browsers.map((browser, index) => (
            ...
            >
              ...
              {/* Icon */}
              <div className="text-5xl mb-2">{getBrowserIcon(browser.id)}</div>

              {/* Name */}
              <div className="text-white font-medium text-center text-sm">{browser.name}</div>
            </button>
          ))}
        </div>
